...
export default function NuevaVisitaPage() {
    ...
    const fetchAgents = async () => {
        ...
        try {
            // 1. Get current user's organizationId
            const userDocRef = doc(db, "users", auth.currentUser.uid);
            const userDocSnap = await getDoc(userDocRef);
            const userData = userDocSnap.data();
            const organizationId = userData?.organizationId;

            // 2. Query users by organizationId
            let q;
            if (organizationId) {
                q = query(collection(db, "users"), where("organizationId", "==", organizationId));
            } else {
                // Fallback: If no organizationId, only show current user to prevent data leak
                q = query(collection(db, "users"), where("__name__", "==", auth.currentUser.uid));
            }
            ...
            const users = snapshot.docs
                .map(doc => ({
                    id: doc.id,
                    nombre: doc.data().displayName || doc.data().email || "Sin Nombre",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await visitasService.createVisita({
                ...
                agenteNombre: selectedAgent.nombre || "Sin Nombre",
                fechaHora,
        ...
        <div className="p-6 max-w-4xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
                ...
                <div>
                    ...
                    <select
                        required
                        className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-black bg-white"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            >
                                ...
                                {agents
                                    .filter((agent, index, self) => index === self.findIndex((t) => t.id === agent.id))
                                    .map(agent => (
