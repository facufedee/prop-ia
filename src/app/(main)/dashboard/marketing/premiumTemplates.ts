// Pre-written templates ready to load with one click from the Marketing UI.
// Content matches the copy validated in scripts/seed_marketing_templates.js.

const saasWrapper = (content: string) => `
...
const RAW_TEMPLATES: { type: string; name: string; subject: string; content: string }[] = [
    {
        type: 'welcome',
        name: 'Bienvenida',
...
export const PREMIUM_TEMPLATES = RAW_TEMPLATES.map(t => ({
    type: t.type,
    name: t.name,
    subject: t.subject,
